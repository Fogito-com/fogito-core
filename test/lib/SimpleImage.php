...
class SimpleImage
{
    ...
    {
        $create = imagecreatefrompng( $watermark );
        $rotatedImage = imagerotate( $create , 45 , imagecolorallocatealpha( $create , 255 , 255 , 255 , 127 ) );

        $wx = imagesx( $rotatedImage );
        $wy = imagesy( $rotatedImage );

        $ww = ceil( $wx * 1 );
        $wh = ceil( $wy * 1 );

        $w_left = ($this->width - $ww) / 2;
        $w_bottom = ($this->height - $wh) / 2;

        imagecopy( $this->image , $rotatedImage , $w_left , $w_bottom , 0 , 0 , $ww , $wh );
        return $this;
    }
